...
public class ProductRelatedDataDaoSqls {
    ...
    public static final String SELECT_RESERVATION_USER_COMMENTS_IMAGES =
        "SELECT r_img.id AS reservation_user_comment_image_id, r_img.reservation_user_comment_id, "
            + "r_img.file_id FROM reservation_user_comment AS r_comment LEFT OUTER JOIN "
            + "reservation_user_comment_image AS r_img ON r_comment.id = r_img.reservation_user_comment_id "
            + "WHERE r_comment.id = :commentId";
